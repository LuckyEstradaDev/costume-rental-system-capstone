...
export function BundleCard({bundle}: {bundle: IBundle}) {
  ...
  const bundleSlug = buildBundleSlug(bundle.name, bundle._id);
  const detailHref = bundleSlug
    ? `/dashboard/browse/bundles/bundle/${bundleSlug}`
    : undefined;
    ...
    <Card className="group cursor-pointer overflow-hidden border border-border/60 bg-background py-0 transition-all hover:-translate-y-1 hover:shadow-lg">
      <div className="relative aspect-square overflow-hidden bg-muted">
        {detailHref ? (
          ...
          <Image src={imageSrc} alt={bundle.name} fill className="object-cover" />
        )}
        <div className="absolute left-2 top-2 flex flex-col gap-1">
          <Badge className="bg-black/70 text-white backdrop-blur">Bundle</Badge>
      ...
      <div className="space-y-1 p-3">
        {detailHref ? (
          ...
          <p className="line-clamp-2 text-sm font-medium leading-tight">{bundle.name}</p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1 text-sm text-muted-foreground">
          {bundle.rentalPrice > 0 && (
            <span className="flex items-center gap-1">
              <CalendarClock className="size-4 text-primary" />
              <span className="font-semibold text-primary">₱{bundle.rentalPrice.toLocaleString()}</span>
            </span>
          )}
          {bundle.price > 0 && (
            <span className="flex items-center gap-1">
              <CreditCard className="size-4 text-primary" />
              <span className="font-semibold text-primary">₱{bundle.price.toLocaleString()}</span>
            </span>
          )}
        </div>
...
function buildBundleSlug(name: string, bundleId?: string) {
  if (!bundleId) return "";

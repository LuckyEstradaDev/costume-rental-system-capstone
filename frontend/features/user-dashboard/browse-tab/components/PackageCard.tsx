    ...
    <Card className="group cursor-pointer overflow-hidden border border-border/60 bg-background py-0 transition-all hover:-translate-y-1 hover:shadow-lg">
      <div className="relative aspect-square overflow-hidden bg-muted">
        {detailHref ? (
          ...
          <Image src={imageSrc} alt={packageItem.name} fill className="object-cover" />
        )}
        <div className="absolute left-2 top-2 flex flex-col gap-1">
          <Badge className="bg-black/70 text-white backdrop-blur">Package</Badge>
      ...
      <div className="space-y-1 p-3">
        {detailHref ? (
          ...
          <p className="line-clamp-2 text-sm font-medium leading-tight">{packageItem.name}</p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1 text-sm text-muted-foreground">
          {packageItem.rentalTotal != null && packageItem.rentalTotal > 0 && (
            <span className="flex items-center gap-1">
              ...
              <span className="font-semibold text-primary">₱{packageItem.rentalTotal.toLocaleString()}</span>
            </span>
          )}
          {packageItem.purchaseTotal != null && packageItem.purchaseTotal > 0 && (
            <span className="flex items-center gap-1">
              <CreditCard className="size-4 text-primary" />
              <span className="font-semibold text-primary">₱{packageItem.purchaseTotal.toLocaleString()}</span>
            </span>
          )}
        </div>

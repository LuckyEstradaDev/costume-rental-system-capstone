...
export const fetchBundlesService = async (): Promise<IBundle[]> => {
  try {
    const res = await api.get<IBundle[]>("/api/packages");
    return res.data;
  } catch (error) {
...
export const fetchBundleById = async (bundleId: string): Promise<IBundle> => {
  const res = await api.get<IBundle>(`/api/packages/${bundleId}`);
  return res.data;
};
...
export const createBundleService = async (
  ...
  try {
    const res = await api.post<IBundle>("/api/packages", bundle);
    return res.data;
  } catch (error) {
...
export const updateBundleService = async ({
  ...
  const res = await api.patch<IBundle>(`/api/packages/${bundleId}`, updateData);
  return res.data;
};
...
export const deleteBundleService = async (
  ...
  const res = await api.delete<IBundle>(`/api/packages/${bundleId}`);
  return res.data;
};

...
function OutfitReviews({outfit, reviews}: {outfit: IOutfit; reviews: IReview[]}) {
      ...
      <div className="mt-4 space-y-3">
        {reviews.length ? (
          reviews.map((review, index) => (
            <div key={review._id ?? `${review.outfitID}-${index}`} className="rounded-lg border bg-background p-3">
    ...
    <article className="space-y-5 rounded-2xl border bg-background p-4 sm:p-6">
      <div className="flex flex-col gap-5 sm:flex-row">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          {detailHref ? (
            <Link href={detailHref} className="text-2xl font-semibold tracking-tight hover:underline">
              {outfit.name}
            </Link>
          ) : (
            <h2 className="text-2xl font-semibold tracking-tight">{outfit.name}</h2>
          )}
          ...
          <div className="flex flex-wrap gap-5 text-sm">
            <span className="font-medium">Buy: {formatPrice(Number(outfit.price))}</span>
            <span className="font-medium">Rent: {formatPrice(Number(outfit.rentalPrice))}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-3 rounded-xl border bg-muted/20 p-4">
          <div className="flex items-center gap-2 text-sm font-medium"><Palette className="size-4 text-muted-foreground" />Colors and sizes</div>
          {outfit.variants.length ? outfit.variants.map((variant: Variant, index) => (
            <div key={variant._id ?? `${variant.color}-${index}`} className="space-y-2 rounded-lg border bg-background p-3">
              <p className="text-sm font-medium">{variant.color || "Unnamed color"}</p>
              <div className="flex flex-wrap gap-2">
                {variant.sizes.map((size, sizeIndex) => (
                  <Badge key={`${size.size}-${sizeIndex}`} variant={size.stock > 0 ? "outline" : "secondary"}>
        ...
        <div className="space-y-3 rounded-xl border bg-muted/20 p-4">
          <div className="flex items-center gap-2 text-sm font-medium"><Ruler className="size-4 text-muted-foreground" />Measurements</div>
          {outfit.variants.flatMap((variant) => variant.sizes.flatMap((size) =>
            Object.entries(size.measurements ?? {}).map(([key, value]) => (
              <div key={`${size.size}-${key}`} className="flex items-center justify-between gap-3 rounded-lg border bg-background px-3 py-2 text-sm">
                <span className="capitalize text-muted-foreground">{key.replace(/([A-Z])/g, " $1")}</span>
                <span className="font-medium">{value} cm</span>
              </div>
            )),
          ))}
          {!outfit.variants.some((variant) => variant.sizes.some((size) => Object.keys(size.measurements ?? {}).length)) && (
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl space-y-10 px-4 py-6 md:px-8 md:py-10">
        <Link href="/dashboard/browse" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
